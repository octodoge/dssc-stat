...
MLE_logn # Log-Normal distribution
```
...
a)
We test $H0: \mu_1 = \mu_2$ against $Ha: \mu_1 \ne \mu_2$
...
$$
with $df = n_1 +n_2 −2 = 43+50-2=91$, the two-sided $P$-value is 0.022 .

b)
From the previous answer, we know that $s=7.51$, $\bar{y_2} − \bar{y_1} = 3.61$ and $se=1.54$.

With $df =n_1 +n_2 −2=91$, a 95% confidence interval for $(\mu_1 −\mu_2)$ is
$$
\left(\bar{y}_2-\bar{y}_1\right) \pm t_{0.025,91}(s e)=3.61 \pm 1.99(1.54) \text {, which is } 3.61 \pm 3.06 \text {, or }(0.55, 6.67) \text {. }
$$

Since the size of the sample and the dispersion is known, confidence intervals can be calculated from p-values, and conversely.

To compare the mean sales, the p-value is more informative since allow a decision about the rejection or maintenance of the previously formulated null hypothesis. In this case we reject $H_0$ because P-value of 0.022 is less than 0.05 and we can infer that $\mu_2>\mu_1$, so menu B works better than menu A.
...
```
We can see that coverage is lower than the required confidence level of `0.95`.
