...
**Solution**  
We simulate three sets of data using a standard normal distribution with $n$ equal 4, 16 and 64
```{r data_generation}
i <- 1:3
n <- 4^(i)
...
We see that the mean tends to get closer to zero as $n$ increases.
```{r mean}
mean(as.numeric(unlist(rn.x[1]))) # n = 4
mean(as.numeric(unlist(rn.x[2]))) # n = 16
mean(as.numeric(unlist(rn.x[3]))) # n = 64
```

Similarly the variance tends to get closer to one as $n$ increases.
```{r variance}
var(as.numeric(unlist(rn.x[1]))) # n = 4
var(as.numeric(unlist(rn.x[2]))) # n = 16
var(as.numeric(unlist(rn.x[3]))) # n = 64
```
...
library(MASS)
for (j in i) {
  hist.scott(as.numeric(unlist(rn.x[j])), col = tr_colors[j], xlim = range(-4,4), 
             ylim = range(0.,0.6), main = paste('n =', n[j]), xlab = 'x')
  curve(dnorm, col = "red", type = 'l', lwd=2, add = T)
}
```

Following the formulation of the Central Limit theorem in Agresti-Kateri's book we calculate the cumulative distribution function for the three datasets and compare it to the actual cumulative distribution of a standard normal distribution.
```{r cdf}
par(mfrow=c(2,2))
for (j in i) {
  plot(ecdf(as.numeric(unlist(rn.x[j]))), main = paste('n =', n[j]), 
       xlim = range(-3,3))
  curve(pnorm, col = "red", type = 'l', lwd=2, add = T)
}
```
