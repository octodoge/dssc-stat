...
TBD (nella formula non posso dire che la population mean è uguale alla sample mean...)
...
$$
\mu_d = {1 \over n} \sum_{i=1}^n d_i = {1 \over n} \sum_{i=1}^n (y_{i1} − y_{i2}) =
{1 \over n} \sum_{i=1}^n y_{i1} - {1 \over n} \sum_{i=1}^n y_{i2} = \mu_1 - \mu_2
$$

#### c.

TBD (continua a calcolare il P-value)

Now we want to do a significance test for the mean $\mu_d$.

We can assume that the differences $\{d_i = y_{i1} − y_{i2},\; i=1,...,8\}$ are approximately normally distributed since the mean response times $y_{i1}$ and $y_{i2}$ are approximately normally distributed for the Central Limit Theorem. But, actually, the significance test for the mean is quite robust and performs well even if this assumption is violated.
