...
y_train = subset(d_train, select=quality)

#d_light
d_light = subset(d_train, select=-density)
x_light = subset(x_train, select=-density)
y_light = y_train
...
#d_clean
d_clean = clean(d_train, train_set=x_train) #3597 rows
x_clean = subset(d_clean, select=-quality)
y_clean = subset(d_clean, select=quality)

#d_light_clean
d_light_clean = clean(d_light, train_set=x_light)
x_light_clean = subset(d_light_clean, select=-quality)
y_light_clean = subset(d_light_clean, select=quality)
```
...
```{r}
means = colMeans(x)
sds = apply(x, MARGIN=2, FUN=sd) #standard deviations
f = function(x) all(x>means-3*sds & x<means+3*sds)
df[apply(df, MARGIN=1, FUN=f),] #4502 rows
```

```{r}
df[,array_di_bool_lungo_quanto_colonna] #tutte righe, alcune colonne
```
