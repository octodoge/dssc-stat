...
3.  $D_{clean}$ is the dataset obtained by the **three-sigma rule of thumb** excluding the observations outside the $[\overline{x}-3\sigma_{x_j},\overline{x}+3\sigma_{x_j}]$ where $x_j$ is the observation coming from the i-feature ($j=1,\dots,11$).

$$\text{codice su come ottenere il dataset}$$

4.  $D_{light,clean}$ is the dataset obtained from $D_{light}$ by applying the **three-sigma rule of thumb**. We decide to *light* it before because we don't want to remove observations coming from the cleaning process on the **density feature**.
