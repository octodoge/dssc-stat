...
```{r}
x_train_light_clean_s <- subset(x_train_clean_s, select = -density) # drop column
```
...
x_test_light_s <- subset(x_test_s, select = -density) # drop column

# TODO remember to change names, because they aren't cleaned, just scaled with another thing
x_test_clean_s <- scale(x_test, center = means_clean, scale = sds_clean)
x_test_light_clean_s <- subset(x_test_clean_s, select = -density) # drop column
```
