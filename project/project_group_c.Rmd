...
fit_lasso <- function(x, y){
  ...
  lasso <- glmnet(x, y, family=gaussian, lambda = cv$lambda.1se)
  coef(lasso)
}
```
...
```{r warning=FALSE, message=FALSE}
set.seed(101)
fit_lasso(x_train_light_s, y_train)
```
...
```{r warning=FALSE, message=FALSE}
set.seed(101)
fit_lasso(x_train_clean_s, y_train)
```
...
```{r warning=FALSE, message=FALSE}
set.seed(101)
fit_lasso(x_train_light_clean_s, y_train)
```
...
x_test_int <- model.matrix(as.formula( ~ .*.), data.frame(x_test))[,-1]
x_test_s <- scale(x_test_int, center = means, scale = sds)

x_test_light_int <- model.matrix(as.formula( ~ .*.), data.frame(x_test_light))[,-1]
