...
Using boxplots we can visualize summary statistics such as mean, median and identify precence of outliers or unusual values.

```{r}
plotboxplot <- function(x, name) {
  return(ggplot(data = data.frame(x), aes(x=x)) + geom_boxplot(outlier.size=1, outlier.alpha = 0.3) + ggtitle(name))
}
