...
```{r}
x_train_int <- model.matrix(as.formula( ~ .*.), data.frame(x_train))[,-1]
...
```{r}
x_train_light_int <- model.matrix(as.formula( ~ .*.), data.frame(x_train_light))[,-1]
...
```{r}
x_train_clean_int <- model.matrix(as.formula( ~ .*.), data.frame(x_train_clean))[,-1]
...
```{r}
x_train_light_clean_int <- model.matrix(as.formula( ~ .*.), data.frame(x_train_light_clean))[,-1]
...
set.seed(101)
lasso = fit_lasso(x_train_s, y_train)
#coef(lasso)
```
...
```{r message=FALSE, warning=FALSE}
set.seed(101)
lasso_light = fit_lasso(x_train_light_s, y_train_light)
#coef(lasso_light)
```
...
```{r message=FALSE, warning=FALSE}
set.seed(101)
lasso_clean = fit_lasso(x_train_clean_s, y_train_clean)
#coef(lasso_clean)
```
...
```{r message=FALSE, warning=FALSE}
set.seed(101)
lasso_light_clean = fit_lasso(x_train_light_clean_s, y_train_light_clean)
...
assess.glmnet(lasso, newx=x_test_s, newy=y_test)$mse
...
assess.glmnet(lasso_light, newx=x_test_light_s, newy=y_test_light)$mse

x_test_s2 <- scale(x_test_int, center = means_clean, scale = sds_clean)
assess.glmnet(lasso_clean, newx=x_test_s2, newy=y_test)$mse

x_test_light_s2 <- scale(x_test_light_int, center = means_light_clean, scale = sds_light_clean)
assess.glmnet(lasso_light_clean, newx=x_test_light_s2, newy=y_test_light)$mse
```
