...
```{r}
d_train_s = d_train
d_train_s[,1:11] = apply(d_train_s[,1:11], MARGIN = 2, scale )
```
2.Rescale for $D_{train}^{light}$
```{r}
d_train_light_s = d_train_light
d_train_light_s[,1:11] = apply(d_train_light_s[,1:11], MARGIN = 2, scale )
```

3.Rescale for $D_{train}^{clean}$
```{r}
d_train_clean_s = d_train_clean
d_train_clean_s[,1:11] = apply(d_train_clean_s[,1:11], MARGIN = 2, scale )
```
4.Rescale for $D_{train}^{light,clean}$
```{r}
d_train_light_clean_s = d_train_light_clean
d_train_light_clean_s[,1:11] = apply(d_train_light_clean_s[,1:11], MARGIN = 2, scale )
```
...
print("Coefficients of the model trained on d_train_s")
