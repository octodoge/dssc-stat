...
*Suppose the sample data distribution of* $\{y_i\} = \{y_1,...,y_n\}$ *is very highly skewed to the right, and we take logs and analyze* $\{xi = log(yi)\}$*.*

a. *Is* $\bar x = log(\bar y)$*? Why or why not?*
b. *Is* $median(\{x_i\}) = log[median(\{y_i\})]$? *Why or why not?*
c. *To summarize* $\{y_i\}$, we find $\bar x$ and then use $exp(\bar x)$*. Show that* $exp(\bar x) = (\begin{matrix} \prod_i y_i \end{matrix})^{1/n}$*, called the geometric mean of* $\{y_i\}$*.*
