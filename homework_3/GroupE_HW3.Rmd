...
bay_est
```
We see that the posterior mean estimate increase with the parameters of the prior distribution.


(b) let's plot the posterior distribution
```{r }
alpha<-1
beta<-1
curve(dbeta(x,alpha+y,beta+n-y),xlab="p", ylab="density",lty=1,lwd=2)#, cex.axis=.5, cex.lab=.6, ann=F)
legend(.7,15,"posterior", lty=1)#c(3,4,1))
```
...
Substituting with $f(y|\pi) = \pi^y (1 - \pi)^{1-y}$, we get that the Jeffreys prior is $\frac{1}{\sqrt{\pi (1-\pi)}}$ which is the beta distribution with hyperparameters $\alpha = \beta = \frac{1}{2}$, $Beta(1/2, 1/2)$.\ Hence, the posterior distributions of $\pi_1$ and $\pi_2$ are $Beta(11.5, 0.5)$, and $Beta(0.5, 1.5)$ respectively.
```{r}
library(HDInterval)
...
&=\lambda^{15} e^{-\lambda \sum_{i=1}^{15}y_i} \frac{2^4}{6}\lambda^{3}e^{-2\lambda}\\
&=\frac{8}{3}\lambda^{18}e^{-\lambda((\sum_{i=1}^{15}y_i) +2)}
\end{aligned}
$$ 
